...
import ereefs.boxrenderer.BreakBox;
import ereefs.boxrenderer.ContentBox;
import ereefs.boxrenderer.ContentBoxImpl;
...
public class Parser {
    ...
    public static interface Resolver {
        public InputStream resolve(String href) throws Exception;
    }
    ...
    private void applyCss(Box box) throws Exception {
        ...
        CssStyleFactory cfac = new CssStyleFactory();
        for(PropertyValue p : properties) {
            CssStyle style = cfac.getCssStyle(p);
            if(style!=null) {
                style.style(box);
            }
        }
    ...
    private void parseCss(Element el) {
        ...
        try {
            cssrules.addAll(CSSParser.parse(cssSource));
        } catch(Exception e) {
